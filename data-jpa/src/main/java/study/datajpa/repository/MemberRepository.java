...
public interface MemberRepository extends JpaRepository<Member, Long>, MemberRepositoryCustom, JpaSpecificationExecutor<Member> {
    ...
    @Query("select m from Member m where m.username = :username and m.age = :age")
        //이름이 없는 네임드 쿼리이기 때문에 실행 시점에 SQL 파싱하면서 에러를 잡아 줌
...
//    Slice<Member> findByAge(int age, Pageable pageable); //Pageable 현재 내가 1페이지, 2페이지...
    ...
    @Query(value = "select m from Member m left join m.team t",
        ...
        //쿼리가 복잡하면 성능 테스트 후 카운트 쿼리 분리해야 함
    Page<Member> findByAge(int age, Pageable pageable); //Pageable 현재 내가 1페이지, 2페이지...

    @Modifying(clearAutomatically = true)
    //@Modifying가 있어야 executeUpdate 해줌, clearAutomatically = true는 쿼리가 나가고 em.clear()기능 자동으로 해줌
    ...
    @EntityGraph(attributePaths = {"team"})
        //JPQL 말고, member도 조회하며 team도 조회하고 싶어 -> EntityGraph는 내부적으로 페치 조인 실행
    List<Member> findAll();
...
//    @EntityGraph("Member.all")
    ...
    //    List<UsernameOnly> findProjectionsByUsername(String username); //인터페이스 기반
    //    List<UsernameOnlyDto> findProjectionsByUsername(String username); //클래스 기반 Projections
    <T> List<T> findProjectionsByUsername(@Param("username") String username, Class<T> type); //동적 Projections

    @Query(value = "select * from member where username = ?", nativeQuery = true)
    Member findByNativeQuery(String username);

    //DTO를 뽑는데 편하게 뽑을 수 있고 네이티브 쿼리고 동적 쿼리는 아니야 이럴 때 씀
